...
class SnapshotContextTest {
  ...
  void setUp() throws NoSuchMethodException, IOException {
    snapshotFile =
        new SnapshotFile(DEFAULT_CONFIG.getOutputDir(), "anyFilePath", SnapshotContextTest.class);
    snapshotContext =
        new SnapshotContext(
            DEFAULT_CONFIG,
            snapshotFile,
  ...
  void shouldMatchSnapshotSuccessfully() {
    ...
    assertThat(snapshotFile.getSnapshots().stream().findFirst().get().raw()).isEqualTo(SNAPSHOT);
  }
  ...
  void shouldNotRenderScenarioNameWhenNull() {
    SnapshotContext snapshotContextWithoutScenario =
        new SnapshotContext(
            ...
            String.class.getDeclaredMethod("toString"),
            "anyObject");
    assertThat(snapshotContextWithoutScenario.resolveSnapshotIdentifier())
  ...
  void shouldFailWhenRunningOnCiWithoutExistingSnapshot() {
    BaseSnapshotConfig ciSnapshotConfig =
    ...
    SnapshotContext ciSnapshotContext =
        new SnapshotContext(
            ciSnapshotConfig,
    ...
    Assertions.assertThatThrownBy(ciSnapshotContext::toMatchSnapshot)
        .hasMessage(
            "Snapshot [java.lang.String.toString] not found. Has this snapshot been committed ?");
  }
  ...
  void shouldAggregateMultipleFailures() {
    ...
    Stream<BiConsumer<Snapshot, Snapshot>> reportingFunctions =
        Stream.of(
            (rawSnapshot, currentObject) ->
                assertThat(currentObject).isEqualTo(rawSnapshot), // assertj
    ...
    Stream<SnapshotReporter> reporters =
        reportingFunctions.map(
            consumer ->
                new SnapshotReporter() {
                  @Override
    ...
    SnapshotContext failingSnapshotContext =
        new SnapshotContext(
            DEFAULT_CONFIG,
            snapshotFile,
            String.class,
